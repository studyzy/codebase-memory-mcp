...
static CBMDefinition *find_def(CBMFileResult *r, const char *label, const char *name) {
    for (int i = 0; i < r->defs.count; i++) {
        ...
        if (d->label && strcmp(d->label, label) == 0 &&
...
static int bases_contain(CBMDefinition *d, const char *want) {
    if (!d->base_classes) return 0;
    for (const char **b = d->base_classes; *b; b++) {
        if (strcmp(*b, want) == 0) return 1;
    }
    return 0;
}
...
static int bases_count(CBMDefinition *d) {
    if (!d->base_classes) return 0;
    int n = 0;
    for (const char **b = d->base_classes; *b; b++) n++;
    return n;
}
...
static CBMDefinition *find_def_flex(CBMFileResult *r, const char *name) {
    CBMDefinition *d;
    if ((d = find_def(r, "Class",     name))) return d;
    if ((d = find_def(r, "Interface", name))) return d;
    if ((d = find_def(r, "Struct",    name))) return d;
...
static int run_inherit_case(const inherit_case_t *tc) {
    CBMFileResult *r = cbm_extract_file(tc->src, (int)strlen(tc->src),
                                        tc->lang, "t", tc->path,
                                        0, NULL, NULL);
    if (!r) {
    ...
    for (int i = 0; tc->expected_bases[i]; i++) {
        if (!bases_contain(cls, tc->expected_bases[i])) {
            printf("  FAIL  [%s] expected base \"%s\" not found in base_classes\n",
                   tc->class_name, tc->expected_bases[i]);
            ok = 0;
        }
    }
    ...
    if (tc->min_base_count > 0) {
        ...
        if (cnt < tc->min_base_count) {
            printf("  FAIL  [%s] base_classes has %d entries, expected >= %d\n",
                   tc->class_name, cnt, tc->min_base_count);
            ok = 0;
        }
    }
...
#define RUN_CASES(table)                                              \
    do {                                                              \
        ...
        for (int _i = 0; _i < _n; _i++) {                            \
            if (run_inherit_case(&table[_i]) != 0) _fail++;           \
        }                                                             \
        if (_fail > 0) {                                              \
...
static const inherit_case_t java_cases[] = {
    ...
    {
        CBM_LANG_JAVA, "Svc.java",
        "public class Box<T> implements Comparable<Box<T>> { public int compareTo(Box<T> o) { return 0; } }",
        "Box",
        {"Comparable", NULL},
        {"extends", "implements", NULL},
        1
    ...
    {
        CBM_LANG_JAVA, "Svc.java",
        "public enum Status implements Displayable { OPEN, CLOSED; public String display() { return name(); } }",
        "Status",
        {"Displayable", NULL},
        {"extends", "implements", NULL},
        1
...
static const inherit_case_t tsx_cases[] = {
    ...
    {
        CBM_LANG_TSX, "ErrorBoundary.tsx",
...
static const inherit_case_t php_cases[] = {
    ...
    {
        CBM_LANG_PHP, "Svc.php",
        "<?php\nabstract class AbstractSvc extends BaseService { abstract public function run(): void; }",
        "AbstractSvc",
        {"BaseService", NULL},
        {"extends", "implements", NULL},
        1
...
static const inherit_case_t kotlin_cases[] = {
    ...
    {
        CBM_LANG_KOTLIN, "Svc.kt",
        "sealed class Result\nclass Success(val value: Any) : Result()\nclass Failure(val err: Throwable) : Result()",
        "Success",
        {"Result", NULL},
        {":", NULL},
        1
    ...
    {
        CBM_LANG_KOTLIN, "Svc.kt",
        "interface Logger { fun log(msg: String) }\nobject ConsoleLogger : Logger { override fun log(msg: String) = println(msg) }",
        "ConsoleLogger",
        {"Logger", NULL},
        {":", NULL},
        1
    ...
    /* ── implements Comparable ───────────────────────────────────── */
    {
        ...
        "override fun compareTo(other: Version) = compareValuesBy(this, other, { it.major }, { it.minor }) }",
        "Version",
        {"Comparable", NULL},
        {":", NULL},
        1
    ...
    /* ── fun interface (SAM) ─────────────────────────────────────── */
    {
        ...
        "class UpperTransformer : Transformer { override fun transform(input: String) = input.uppercase() }",
        "UpperTransformer",
        {"Transformer", NULL},
        {":", NULL},
        1
    ...
    {
        CBM_LANG_KOTLIN, "Svc.kt",
        "annotation class MyAnnotation\nclass AnnotatedClass : BaseClass() { @MyAnnotation fun doWork() {} }",
        "AnnotatedClass",
        {"BaseClass", NULL},
        {":", NULL},
        1
    ...
    /* ── value class (inline, Kotlin 1.5+) ──────────────────────── */
    {
        ...
        "@JvmInline value class UserId(val id: String) : Printable { override fun print() = println(id) }",
        "UserId",
        {"Printable", NULL},
        {":", NULL},
        1
...
static int rust_has_impl(CBMFileResult *r, const char *trait_name, const char *struct_name) {
    for (int i = 0; i < r->impl_traits.count; i++) {
        ...
        if (it->trait_name && strcmp(it->trait_name, trait_name) == 0 &&
            it->struct_name && strcmp(it->struct_name, struct_name) == 0)
            return 1;
    }
    return 0;
}
...
#define RUN_RUST_CASES(table)                                              \
    do {                                                                   \
        ...
        int _n = (int)(sizeof(table) / sizeof(table[0]));                  \
        for (int _i = 0; _i < _n; _i++) {                                 \
            if (run_rust_impl(&table[_i]) != 0) _fail++;                   \
        }                                                                  \
        if (_fail > 0) {                                                   \
...
static const rust_impl_case_t rust_cases[] = {
    ...
    /* ── crate-qualified trait ───────────────────────────────────── */
    {
        ...
        "struct Socket;\nimpl io::Write for Socket { fn write(&mut self, buf: &[u8]) -> usize { buf.len() } }",
        "io::Write", "Socket"
    },
    ...
    /* ── Error trait ─────────────────────────────────────────────── */
    {
        ...
        "impl fmt::Display for AppError { fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result { write!(f, \"{}\", self.msg) } }\n"
    ...
    /* ── Index trait ─────────────────────────────────────────────── */
    {
        ...
        "impl Index<(usize, usize)> for Matrix { type Output = f64; fn index(&self, idx: (usize, usize)) -> &f64 { &self.data[idx.0 * self.cols + idx.1] } }",
        "Index", "Matrix"
    },
    ...
    /* ── serde Serialize / Deserialize (external crate trait) ────── */
    {
        ...
        "impl Serialize for JsonRecord { fn serialize(&self) -> String { format!(\"{{\\\"id\\\":{}}}\", self.id) } }\n"
...
SUITE(extraction_inheritance) {
    ...
    RUN_TEST(inherit_typescript);  /* RED: keyword "extends"/"implements" captured instead of type name */
